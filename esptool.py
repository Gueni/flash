    ...
    def read_flash(self, offset, length, progress_fn=None):
        ...
        while len(data) < length:
            ...
            if len(data) < length and len(p) < self.FLASH_SECTOR_SIZE:
                raise FatalError(
                    'Corrupt data, expected 0x%x bytes but received 0x%x bytes' % (self.FLASH_SECTOR_SIZE, len(p)))
...
class BaseFirmwareImage(object):
    ...
    def verify(self):
        if len(self.segments) > 16:
            raise FatalError('Invalid segment count %d (max 16). Usually this indicates a linker script problem.' % len(
                self.segments))
...
class ESP32FirmwareImage(BaseFirmwareImage):
    ...
    def save(self, filename):
        ...
        with io.BytesIO() as f:  # write file to memory first
            ...
            flash_segments = [copy.deepcopy(s) for s in sorted(self.segments, key=lambda s: s.addr) if
                              self.is_flash_addr(s.addr)]
            ram_segments = [copy.deepcopy(s) for s in sorted(self.segments, key=lambda s: s.addr) if
                            not self.is_flash_addr(s.addr)]
...
def dump_mem(esp, args):
    ...
    settingsprogresdump.setValue(dump_progress, 0)
    with open(args.filename, 'wb') as f:
        for i in range(args.size // 4):
            ...
            if f.tell() % 1024 == 0:
                ...
                print(f.tell() * 100 // args.size)
                settingsprogresdump.setValue(dump_progress, f.tell() * 100 // args.size)
                settingsprogresdump.sync()
            sys.stdout.flush()
    settingsprogresdump.setValue(dump_progress, 0)
    settingsprogresdump.sync()
...
def write_flash(esp, args):
    ...
    settingsprogresswrite.setValue(progressnum, 0)
    ...
    for address, argfile in args.addr_filename:
        ...
        while len(image) > 0:
            ...
            settingsprogresswrite.setValue(progressnum, 100*(seq+1)//blocks)
            settingsprogresswrite.sync()
...
def read_flash(esp, args):
    ...
    settingsread_flash.setValue(read_flash_progress, 0)
    ...
    if args.no_progress:
        ...
        def flash_progress(progress, length):
            ...
            settingsread_flash.setValue(read_flash_progress,int(progress * 100.0 / length ))
            settingsread_flash.sync()
            print(progress*100.0/length)
            padding = '\b' * len(msg)
            if progress == length:
                padding = '\n'
    ...
    t = time.time() - t
    print('\rRead %d bytes at 0x%x in %.1f seconds (%.1f kbit/s)...' % (len(data), args.address, t, len(data) / t * 8 / 1000))
...
def verify_flash(esp, args):
    differences = False
    settingsverif_ini = QSettings("verif_ini.ini", QSettings.IniFormat)
    final_message="n"
    ...
    for address, argfile in args.addr_filename:
        ...
        if digest == expected_digest:
            ...
            final_message = "verify OK (digest matched)"
            continue
        else:
            differences = True
            if getattr(args, 'diff', 'no') != 'yes':
                print('-- verify FAILED (digest mismatch)')
                final_message = "verify FAILED (digest mismatch)"
                continue
    ...
    settingsverif_ini.setValue(verifconst, final_message)
...
def main(custom_commandline=None):
    ...
    becomes ['-b', '115200'].
    """
    parser = argparse.ArgumentParser(description='esptool.py v%s - ESP8266 ROM Bootloader Utility' % __version__,
                                     prog='esptool')
    ...
    def add_spi_connection_arg(parent):
        parent.add_argument('--spi-connection', '-sc',
    ...
    parser_write_flash.add_argument('addr_filename', metavar='<address> <filename>',
    ...
    parser_write_flash.add_argument('--encrypt', help='Encrypt before write ',
                                    action='store_true')
    parser_write_flash.add_argument('--ignore-flash-encryption-efuse-setting',
    ...
    parser_elf2image.add_argument('--output', '-o',
    ...
    parser_elf2image.add_argument('--secure-pad', action='store_true',
                                  help='Pad image so once signed it will end on a 64KB boundary. For ESP32 images only.')
    parser_elf2image.add_argument('--elf-sha256-offset',
    ...
    parser_read_status.add_argument('--bytes', help='Number of bytes to read (1-3)', type=int, choices=[1, 2, 3],
                                    default=2)
    ...
    parser_write_status.add_argument('--non-volatile', help='Write non-volatile bits (use with caution)',
                                     action='store_true')
    parser_write_status.add_argument('--bytes', help='Number of status bytes to write (1-3)', type=int,
    ...
    parser_verify_flash.add_argument('addr_filename',
    ...
    parser_erase_region.add_argument('size', help='Size of region to erase (must be multiple of 4096)',
                                     type=arg_auto_int)
