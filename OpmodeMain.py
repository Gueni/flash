...
QSS_style               = "Theme/qssthemes/LightBlue/stylesheet.qss"
settingsprogresswrite   = "Dependecies/settingsprogresswrite.ini"
...
class MyprogressbarThreadflashall(QThread):
    ...
    def run(self):
        cnt                         = 0
        while cnt <= 100:
            settingsprogresswrite   = QSettings(settingsprogresswrite, QSettings.IniFormat)
...
class ESPToolGUIApp(QtWidgets.QMainWindow, opmodeui.Ui_MainWindow):
    ...
    def __init__(self, parent=None):
        ...
        self.setupUi(self)
        self.setWindowIcon(QtGui.QIcon(self.bundle_dir + espLogo_png))
    ...
    def setactionLight_Blue(self):
        ...
        self.settingstyle = QSettings(settingsstyle, QSettings.IniFormat)
        self.setStyleSheet(open(QSS_style, "r").read())
        self.settingstyle.setValue(SETTINGS_style, QSS_style)
        self.settingstyle.sync()
        self.label.setStyleSheet(open(self.settingstyle.value(SETTINGS_style, type=str), "r").read())
        self.label_2.setStyleSheet(open(self.settingstyle.value(SETTINGS_style, type=str), "r").read())
    ...
    def expandcollaps(self):
        ...
        if testheight == 296 and testwidth == 781:
            ...
            self.pushButtoncollapsexpand.setIcon(QIcon(expand_png))
            self.pushButtoncollapsexpand.setIconSize(QSize(24, 15))
    ...
    def erase(self):
        ...
        if os.name == 'nt':
            if self.chip == 'ESP32':
                self.process.start(
                    'python ' +self.bundle_dir + 'Dependecies/esptool.py --chip esp32 --port {0} --baud {1} erase_flash'.format(self.port,
                                                                                                           self.baudRate))
            if self.chip == 'ESP8266':
                self.process.start(
                    'python ' +self.bundle_dir + 'Dependecies/esptool.py --chip esp8266 --port {0} --baud {1} erase_flash'.format(self.port,
                                                                                                             self.baudRate))
