...
EFUSES = [
    ...
    ('BLK3_PART_RESERVE', "calibration", 0, 3, 1 << 14, 10, 3, "flag",
...
class EfuseField(object):
    ...
    def __init__(self, parent, register_name, category, block, word, mask, write_disable_bit, read_disable_bit,
                 efuse_type, description):
...
class EfuseKeyblockField(EfuseField):
    ...
    def burn_words(self, words, word_offset=0):
        ...
        if warnings_after & ~warnings_before != 0:
            print("WARNING: Burning efuse block added coding scheme warnings 0x%x -> 0x%x. Encoding bug?" % (
...
def summary(esp, efuses, args):
    ...
    print(ROW_FORMAT.replace("-50", "-12") % (
    ...
    for category in set(e.category for e in efuses):
        ...
        for e in (e for e in efuses if e.category == category):
            ...
            data = [(e.register_name, e.description, value,perms,raw)]
            # append all rows

            for row in data:
                cur.execute('''INSERT INTO samples(Field1, Field2,Field3,Field4,Field5) VALUES(?,?,?,?,?)''', row)
                con.commit()
...
def burn_efuse(esp, efuses, args):
    ...
    if efuse.efuse_type == "flag":
        if args.new_value not in [None, 1]:
            raise esptool.FatalError(
                "Efuse %s is type 'flag'. New value is not accepted for this efuse (will always burn 0->1)" % efuse.register_name)
        args.new_value = 1
        if old_value:
    ...
    if args.new_value & (efuse.mask >> efuse.shift) != args.new_value:
        raise esptool.FatalError("Value mask for efuse %s is 0x%x. Value 0x%x is too large." % (
            efuse.register_name, efuse.mask >> efuse.shift, args.new_value))
    if args.new_value | old_value != args.new_value:
        print("WARNING: New value contains some bits that cannot be cleared (value will be 0x%x)" % (
                old_value | args.new_value))

    confirm("Burning efuse %s (%s) 0x%x -> 0x%x" % (
        efuse.register_name, efuse.description, old_value, args.new_value | old_value), args)
...
def burn_key(esp, efuses, args):
    ...
    if size != num_bytes:
        raise esptool.FatalError(
            "Incorrect key file size %d. Key file must be %d bytes (%d bits) of raw binary key data." %
...
def burn_block_data(esp, efuses, args):
    ...
    if efuses.coding_scheme == CODING_SCHEME_34:
        ...
        if len(data) % 6 != 0:
            raise RuntimeError(
                "Device has 3/4 Coding Scheme. Can only write data lengths which are a multiple of 6 (data is %d bytes)" % len(
                    data))
    ...
    confirm("Burning efuse %s (%s) with %d bytes of data at offset %d in the block" % (
        efuse.register_name, efuse.description, len(data), offset), args)
...
def burn_custom_mac(esp, efuses, args):
    ...
    if ver != new_ver:
        confirm("Burning efuse %s (%s) 0x%x -> 0x%x" % (ver_fuse.register_name, ver_fuse.description, ver, new_ver),
                args)
        ver_fuse.burn(new_ver)
    confirm("Burning efuse %s (%s) %s -> %s" % (
        mac_fuse.register_name, mac_fuse.description, hexify(old_mac, ":"), hexify(new_mac, ":")), args)
...
def main():
    parser = argparse.ArgumentParser(description='espefuse.py v%s - ESP32 efuse get/set tool' % esptool.__version__,
                                     prog='espefuse')
    ...
    def add_force_write_always(p):
        p.add_argument('--force-write-always',
                       help="Write the efuse even if it looks like it's already been written, or is write protected. " +
    ...
    p.add_argument('new_value', help='New value to burn (not needed for flag-type efuses', nargs='?',
    ...
    p.add_argument('efuse_name', help='Name of efuse register to burn',
                   choices=[efuse[0] for efuse in EFUSES if
                            efuse[6] is not None])  # only allow if read_disable_bit is not None
